...
void CFmodSound::CreateBGSound(int nCount, wstring *SoundFileName)
{
	// 효과 사운드 생성
	m_nBGSoundCount = nCount;
	m_ppBGSound = new FMOD_SOUND*[nCount];
	...
	for (int i = 0; i < nCount; i++)
	{
		char* SoundFile = (char *)malloc(256);
		wcstombs(SoundFile, SoundFileName[i].c_str(), 256);
		FMOD_System_CreateSound(m_pSystem, SoundFile, FMOD_LOOP_NORMAL, 0, &m_ppBGSound[i]);
	}
}



void CFmodSound::CreateEffectSound(int nCount, wstring *SoundFileName)
{
	// 백 그라운드 사운드 생성
	m_nEFSoundCount = nCount;
	m_ppEFFSound = new FMOD_SOUND*[nCount];
	...
	for (int i = 0; i < nCount; i++)
	{
		char* SoundFile = (char *)malloc(256);
		wcstombs(SoundFile, SoundFileName[i].c_str(), 256);
		FMOD_System_CreateSound(m_pSystem, SoundFile, FMOD_DEFAULT, 0, &m_ppEFFSound[i]);
	}
