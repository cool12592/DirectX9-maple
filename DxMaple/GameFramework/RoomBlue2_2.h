...
class RoomBlue2_2 :public IScene
{
public:
	RoomBlue2_2() {}
	virtual ~RoomBlue2_2() {}
	...
	virtual void initialize() override
	{
		...
		//////////////////////////////////////////////////////////////
		ifstream data("GameData-bluemap2.json", ifstream::binary); //µ•¿Ã≈Õø° ¥Ÿ ¿–æÓø»
		...
		for (auto obj : arr)
		{
			...
			else if (jsonName == 1)
			{
				...
				if (doorEntity_.lock() == nullptr)
				{
					...
					entity->addComponent<Door>(L"RoomBlue3");
					entity->getComponent<Transform>().position_ = _vec3(X, Y + 50.f, -1.f);
					entityManager_->insertEntity(entity);
					doorEntity_ = entity;


					if (SceneManager::getInstance()->pastSceneName_ == L"RoomBlue3")
					{
				...
				else
				{
					...
					if (SceneManager::getInstance()->pastSceneName_ == L"RoomBlue2")
					{
		...
		entity->addComponent<CommonEnemy>(L"Ω∫ƒÃ∑π≈Ê");
